...
# generate encodings
del X_train; del X_val; del train_loader; del val_loader; gc.collect()
if args.symmetric:
	...
	del X; del test_loader; gc.collect()
else:
	chunk = 20
	encodings = []
	with np.load(os.path.join(args.data_dir, "L{:d}".format(args.L_test), "aggregate", data_filename), mmap_mode="r") as states_L:
		for i in range(0, len(temperatures), chunk):
			X_chunk = states_L["test"][i:i+chunk,:args.n_test].reshape((-1, states_L["test"].shape[2]))
			T_chunk = np.repeat(temperatures[i:i+chunk], args.n_test)[:,None]
			test_loader = DataLoader(TensorDataset(torch.as_tensor(X_chunk), torch.as_tensor(T_chunk)), batch_size=args.val_batch_size, shuffle=False, drop_last=False, num_workers=8)
			encodings.append( trainer.encode(test_loader) )
			del X_chunk; del test_loader; gc.collect()
...
#from process import calculate_critical_temperatures
#dirs = results_dir.split("/")
#calculate_critical_temperatures(dirs[0], dirs[1], dirs[2], args.L_test, [args.n_test], N=args.n_train_val, fold=args.fold, seed=args.seed)
